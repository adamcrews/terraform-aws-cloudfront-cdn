...
module "logs" {
  source                   = "git::https://github.com/cloudposse/terraform-aws-log-storage.git?ref=tags/0.7.0"
  enabled                  = var.enabled
  namespace                = var.namespace
  stage                    = var.stage
  environment              = var.environment
...
resource "aws_cloudfront_distribution" "default" {
  ...
  dynamic ordered_cache_behavior {
    ...
    content {
      ...
      forwarded_values {
        headers      = ordered_cache_behavior["forwarded_values"]["headers"]
        query_string = ordered_cache_behavior["forwarded_values"]["query_string"]
        cookies {
          forward = ordered_cache_behavior["forwarded_values"]["cookies"]["forward"]
        }
      }
    }
  }
